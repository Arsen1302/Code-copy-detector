class Solution:
    def solution_491_1_1(self, graph: List[List[int]]) -> List[int]:
        n=len(graph)
        status=[0]*(n)
        res=[]
        
        def solution_491_1_2(i):# this function will check is there any loop, cycle and i is a part of that loop,cycle 
            ...
            for j in graph[i]:
                if  solution_491_1_2(j):# if loop detected return True
                    return True
        ...
        for i in range(n):
            if not solution_491_1_2(i): #if no loop detected this node is safe 
                res.append(i)
        return res