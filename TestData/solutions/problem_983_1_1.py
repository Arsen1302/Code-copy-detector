class Solution:
    def solution_983_1_1(self, grid: List[List[int]]) -> int:
        rows, cols = len(grid), len(grid[0])
        
        dp = [[[0]*(cols + 2) for _ in range(cols + 2)] for _ in range(rows + 1)]
        
        def solution_983_1_2(row, col_r1, col_r2):
            res = 0
        ...
        for row in reversed(range(rows)):
            for col_r1 in range(min(cols, row + 2)):
                for col_r2 in range(max(0, cols - row - 1), cols):
                    ...
                    dp[row][col_r1 + 1][col_r2 + 1] = reward + solution_983_1_2(row, col_r1, col_r2)
