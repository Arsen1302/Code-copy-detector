class Solution:
    def solution_1725_2_1(self, s: str, k: int, minLength: int) -> int:
        n = len(s)
        primes = ['2', '3', '5', '7']
        ...
        @cache
        # solution_1725_2_2(i, kk) means number of ways to partition s[ids[i]:n] into kk partitions
        def solution_1725_2_2(i, kk):
            ...
            for j in range(i+1, m-kk+2):
                if ids[j]-ids[i] >= minLength:
                    res += solution_1725_2_2(j, kk-1)
            
            return res % (10**9+7)
        
        return solution_1725_2_2(0, k)