class Solution:
    def solution_345_4_1(self, trees: List[List[int]]) -> List[List[int]]:
        def solution_345_4_2(point1, point2, point3):
        ...
        upper, lower = [], []
        for point in trees:
            while len(upper) > 1 and solution_345_4_2(upper[-2], upper[-1], point) > 0:
                upper.pop()
            while len(lower) > 1 and solution_345_4_2(lower[-2], lower[-1], point) < 0:
                lower.pop()
