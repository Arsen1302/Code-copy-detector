class Solution:
    def solution_681_3_1(self, n: int, lamps: List[List[int]], queries: List[List[int]]) -> List[int]:
        def solution_681_3_2(i, j, dRow, dCol, dDiagS, dDiagP):
        ...
        def solution_681_3_3(i, j, dRow, dCol, dDiagS, dDiagP):
            dRow[i] -= 1
            dCol[j] -= 1
            dDiagS[i + j] -= 1
            dDiagP[i - j] -= 1
        ...
        for i in range(len(queries)):
            # solution_681_3_2 if it's lighted up
            if solution_681_3_2(queries[i][0], queries[i][1], dRow, dCol, dDiagS, dDiagP):
                ans.append(1)
                # solution_681_3_2 for lamp at 9 blocks
                for x in directions:
                    if (queries[i][0]+x[0], queries[i][1]+x[1]) in my_set:
                        solution_681_3_3(queries[i][0]+x[0], queries[i][1]+x[1], dRow, dCol, dDiagS, dDiagP)
