class Solution:
    def solution_1101_2_1(self, nums: List[int], l: List[int], r: List[int]) -> List[bool]:
        out=[]
        for i, j in zip(l, r):
            out.append(self.solution_1101_2_2(nums[i:j+1]))
        return out
        
    def solution_1101_2_2(self, arr: List[int]) -> bool:
        minArr = min(arr)
        maxArr = max(arr)
