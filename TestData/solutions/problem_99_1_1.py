class Solution:
    def solution_99_1_1(self, board: List[List[str]], words: List[str]) -> List[str]:
        m = len(board)
        n = len(board[0])
        res = []
        ...
        for word in words:
            ...
            if not f:
                continue
            if self.solution_99_1_2(word, m, n, board, d):
                res.append(word)
        return res
    
    def solution_99_1_2(self, word, m, n, board, d) -> bool:
        if word[:4] == word[0] * 4:
