class Solution:
    def solution_1485_1_1(self, startAt: int, moveCost: int, pushCost: int, targetSeconds: int) -> int:
        def solution_1485_1_2(minutes, seconds): # Calculates cost for certain configuration of minutes and seconds
        ...
        for m in range(100): # Check which [mm:ss] configuration works out
            for s in range(100):
                if m * 60 + s == targetSeconds: 
                    ans = min(ans, solution_1485_1_2(m, s))
        return ans