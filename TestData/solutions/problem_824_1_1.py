class Solution:
def solution_824_1_1(self, grid: List[List[int]]) -> int:
    
    def solution_824_1_2(i,j):
        if grid[i][j]==1:
            return True
        if i<=0 or i>=m-1 or j<=0 or j>=n-1:
            return False
        grid[i][j]=1
        up=solution_824_1_2(i-1,j)
        down=solution_824_1_2(i+1,j)
        left=solution_824_1_2(i,j-1)
        right=solution_824_1_2(i,j+1)
    ...
    for i in range(1,m-1):
        for j in range(1,n-1):
            ...
            if grid[i][j]==0 and solution_824_1_2(i,j):
                c+=1
    return c