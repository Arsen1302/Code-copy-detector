        ...
        def solution_163_1_2(r,c):
            ...
            for i in range(4): 
                ...
                if 0<=new_row<m and 0<=new_col<n and grid[new_row][new_col]>grid[r][c]: 
                    ans = max(ans, solution_163_1_2(new_row, new_col) + 1 )  # finding the max length of valid path from the current cell                                      
            return ans
        
        return max(solution_163_1_2(r,c) for r in range(m) for c in range(n))