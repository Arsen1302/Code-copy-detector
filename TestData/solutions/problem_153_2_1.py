class Solution:
    def solution_153_2_1(self, nums: List[int]) -> List[int]:
        ...
        desc_nums, counts = self.solution_153_2_2(indexed_nums, counts) # the helper function returns the descendingly sorted indexed_nums and the counts
        return counts
	
	
    def solution_153_2_2(self, nums, counts):
        if len(nums) == 1:
            return nums, counts

        cur_stack = []
        left, counts = self.solution_153_2_2(nums[: len(nums) // 2], counts)
        right, counts = self.solution_153_2_2(nums[len(nums) // 2 :], counts)
