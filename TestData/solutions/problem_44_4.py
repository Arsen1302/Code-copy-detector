class Solution:
    def solution_44_4(self, s: str) -> List[List[str]]:
        dp = []
        n = len(s)
        ...
        for i in range(n-1,-1,-1):
            for j in range(i+1,n+1):
                ...
                if curr == curr[::-1]:  # if substring is palindrome
                    ...
                    # Consider first element of each solution_44_4 is curr then add curr in the front of all partitions of string s[j:]  , which are already stored in dp[j] 
                    for e in dp[j]:   
                        dp[i].append ([curr] + e)
